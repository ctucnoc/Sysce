...
public class SysceExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleGenericClientException(SysceGenericClientException ex) {
		SysceError sysceError = new SysceError(ex.getHttpStatus(), SysceConstant.PREFIX_CLIENT_ERROR);
		sysceError.setMessage(ex.getMessage());
	...
	protected ResponseEntity<Object> handleError(Exception ex) {
		SysceError sysceError = new SysceError(HttpStatus.INTERNAL_SERVER_ERROR,
				SysceConstant.PREFIX_SERVER_ERROR + SysceConstant.INTERNAL_SERVER_ERROR);
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
		...
		return buildResponseEntity(new SysceError(HttpStatus.BAD_REQUEST,
				SysceConstant.PREFIX_CLIENT_ERROR + SysceConstant.BAD_REQUEST, error, ex));
	}
	...
	protected List<SysceSubError> fillValidationErrorsFrom(MethodArgumentNotValidException argumentNotValid) {
		List<SysceSubError> subErrorCollection = new ArrayList<>();
		...
		argumentNotValid.getBindingResult().getFieldErrors().stream().forEach((objError) -> {
			SysceSubError sysceSubError = new SysceValidationError(objError.getObjectName(), objError.getField(),
